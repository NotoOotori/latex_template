...
\begin{enumerate}
    \item 如果$x\in S$, 则$x\depend S$;
    \item 若$x\depend S$, 则存在有限子集$F\subseteq S$使得$x\depend F$;
    \item 如果$x\depend S$并且对于每个$y\in S$都有$y\depend T$, 那么有$x\depend T$;
    \item (Steinitz对换公理) 如果$x\depend S$但是$x\ndepend S-\{y\}$, 则$y\depend (S-\{y\})\cup\{x\}$,
\end{enumerate}
...
\begin{proposition}[{{\parencite[123, Lemma 1]{jacobson_basic_1989}}}]
...
\begin{theorem}[{{\parencite[123, Theorem 3.12]{jacobson_basic_1989}}}]
    设$\Delta$为$X$上的相关关系, 则存在$X$的一个基, 并且$X$的任意两个基具有相同的基数\footnotemark .
\end{theorem}
\footnotetext{这里我们不给出基数的正式定义, 只定义两个非空集合\emph{具有相同的基数}当且仅当存在两个集合之间的双射, 并定义空集与且仅与空集\emph{具有相同的基数}.}
...
\begin{proof}
    ...
    先考虑有限的情况, 利用归纳法证明如果$B$有限, 则$C$有限, 并且$B$与$C$具有相同的基数.

    再考虑无限的情况, 当$B$无限时, 由有限情况的结论知$C$一定无限, 还需要证明$B$与$C$具有相同的基数.
\end{proof}

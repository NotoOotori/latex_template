...
\begin{propositionnoproof}[第三同构定理{{\tjucite[126, Theorem 2.12 (ii)][\thm III.2.12]{hungerford_algebra_1974}}}]\label{prop:thirdisomorphism}
...
\begin{proposition}[{{\tjucite[8, Proposition 1.11][\prop 1.11]{atiyah_introduction_1969}}}]\label{prop:prime}\leavevmode
  \begin{enumerate}
...
\footnotetext{Zorn引理的叙述如下: 如果一个非空偏序集的每个全序子链都有上界, 那么这个偏序集存在极大元. 在\axiom{ZF}公理系统下Zorn引理与选择公理\axiom{AC}等价\tjucite[10, Theorem 2.2][\thm 2.2]{herrlich_axiom_2006}.}

\begin{theorem}[{{\tjucite[3, Theorem 1.3][\thm 1.3]{atiyah_introduction_1969}}}]\label{thm:maxideal}
...
\begin{proposition}[{{\tjucite[8, Exercise 1.12; 9, Exercise 1.13][\ex 1.12 \and 1.13]{atiyah_introduction_1969}}}]\label{prop:idealop}
...
\begin{proposition}[{{\tjucite[9, Proposition 1.14][\prop 1.14]{atiyah_introduction_1969}}}]\label{prop:scheinnullstellensatz}
...
\begin{proposition}[{{\tjucite[10, Proposition 1.17][\prop 1.17]{atiyah_introduction_1969}}}]\label{prop:extencontrcorr}
...
\begin{proposition}[{{\tjucite[10, Exercise 1.18][\ex 1.18]{atiyah_introduction_1969}}}]\label{prop:extencontrop}
...
有自然同态$f\colon A\to S^{-1}A$将$x$映到$x{\divslash}1$, 注意$f$不一定是单射. 分式环有如下的\emph{泛性质}\tjucites[81, Theorem 11.3][\thm 11.3]{altman_term_2017}[37, Proposition 3.1][\prop 3.1]{atiyah_introduction_1969}, 给定环$A$, 乘法闭集$S$与之前定义的自然同态$f\colon A\to S^{-1}A$, 对于任意的环$B$以及将$S$中元素映到单位的环同态$\phi\colon A\to B$, 存在唯一的环同态$\psi\colon S^{-1}R\to B$使得$\psi f=\phi$成立, 并且满足该泛性质的环$S^{-1}A$在同构意义下唯一.
...
\begin{proposition}[{{\tjucite[41, Proposition 3.11][\prop 3.11]{atiyah_introduction_1969}}}]\label{prop:fracideal}
...
\begin{proposition}[{{\tjucite[43, Exercise 3][\chap 3, \ex 3]{atiyah_introduction_1969}}}]\label{prop:multilocalization}
...
设$N$是$B$-模, 如果有环同态$f\colon A\to B$, 则$N$上有自然的$A$-模结构$ay\coloneq f(a)y$, 其中$y\in N, a\in A$. 特别地, 如果考虑$N=B$并赋有典范的$B$-模结构, 则称$B$及同态$f\colon A\to B$为$A$-\emph{代数}.

与有限维线性空间上的线性变换类似, 有限生成模上的模自同态也可以由矩阵表示, 虽然矩阵表示就算给定了生成元也不唯一, 但是我们仍有如下命题, 这是环上的Cayley-Hamilton定理的简单应用.

\begin{proposition}[{{\tjucite[21, Proposition 2.4][\prop 2.4]{atiyah_introduction_1969}}}]\label{prop:fgmoduleendomorphism}
...
\begin{proposition}[{{\tjucite[28, Proposition 2.16][\prop 2.16]{atiyah_introduction_1969}}}]\label{prop:fgmodule}
...
\begin{proposition}[{{\tjucite[30][\page{30}]{atiyah_introduction_1969}}}]\label{prop:fgalgebra}
...
整相关与整元是代数中非常重要的概念, 我们来简要一下并证明其与模之间的关系. 设$B$为环$A$的子环, 如果$x\in A$是某个以$B$中元素为系数的首一多项式的根, 即满足$x^n+b_1x^{n-1}+\dotsb +b_n=0$, 其中$b_1, \dotsc, b_n\in B$, 则称$x$是$B$的\emph{整元}. 特别地, 域上的代数元即为域上的整元.

\begin{proposition}[{{\tjucite[59, Proposition 5.1][\prop 5.1]{atiyah_introduction_1969}}}]\label{prop:integraldependence}
...
\begin{proposition}[{{\tjucite[74, Proposition 6.1][\prop 6.1]{atiyah_introduction_1969}}}]\label{prop:chaincondition}
...
\begin{proposition}[{{\tjucite[75, Proposition 6.2][\prop 6.2]{atiyah_introduction_1969}}}]\label{prop:noethermodule}
...
\begin{proposition}[{{\tjucite[75, Proposition 6.3][\prop 6.3]{atiyah_introduction_1969}}}]\label{prop:noethermoduleexactsequence}
  设
...
\begin{proposition}[{{\tjucite[76, Proposition 6.5][\prop 6.5]{atiyah_introduction_1969}}}]\label{prop:fgmoduleovernoetherring}
...
这一小节中我们将进一步研究Noether环的性质. 前文提到我们的主要研究对象是域的有限生成整环, 我们将证明这些环都是Noether环. 准素分解是交换代数中一个重要的技术, Noether环的理想都可以在某种意义下唯一分解为若干个准素理想的交, 我们将在\ref{sec:ag-ideal}中看到其几何意义.

% \begin{proposition}[{{\tjucite[74, Proposition 6.1; 75, Proposition 6.2]{atiyah_introduction_1969}}}]\label{prop:noetherring}
%     对于环$A$, 以下三个命题等价.
%     \begin{enumerate}
...
\begin{proposition}[{{\tjucite[76, Proposition 6.6; 80, Proposition 7.3][\prop 6.6 \and 7.3]{atiyah_introduction_1969}}}]\label{prop:noetherring2}
  设$A$为Noether环, 则
  \begin{enumerate}
...
\begin{theorem}[Hilbert基定理{{\tjucites[81, Theorem 7.5][\thm 7.5]{atiyah_introduction_1969}[201, Theorem 1][\chap IV, \sec 1, \thm 1]{zariski_commutative_1958}}}]\label{thm:hilbertbasis}
...
准素分解是对环的理想的分解, 由唯一分解整环中元素的分解推广而来, 素元推广到素理想, 而素元的幂次推广到我们即将介绍的准素理想.\clearpage
...
\begin{propositionnoproof}[{{\tjucite[50, Proposition 4.1; 51, Lemma 4.3][\prop 4.1 \and \lem 4.3]{atiyah_introduction_1969}}}]\label{prop:primary}\leavevmode
  \begin{enumerate}
...
\begin{theorem}[Lasker–Noether定理{{\tjucites[83, Theorem 7.13][\thm 7.13]{atiyah_introduction_1969}[209, Theorem 4][\chap IV \sec 4 \thm 4]{zariski_commutative_1958}}}]\label{thm:laskernoether}
...
\begin{propositionnoproof}[{{\tjucite[51, Lemma 4.4][\lem 4.4]{atiyah_introduction_1969}}}]\label{prop:idealquotprimary}
...
\begin{theorem}[{{\tjucite[52, Theorem 4.5][\thm 4.5]{atiyah_introduction_1969}}}]\label{prop:primuniq1}
...
\begin{proposition}[{{\tjucite[52, Proposition 4.6][\prop 4.6]{atiyah_introduction_1969}}}]\label{prop:isolatedprime}
...
\begin{proposition}[{{\tjucite[53, Proposition 4.8; 54, Proposition 4.9][\prop 4.8 \and 4.9]{atiyah_introduction_1969}}}]\label{prop:primlocal}设$S$为环$A$的乘法闭集, 考虑典范同态$A\to S^{-1}A$.
  \begin{enumerate}
...
\begin{theorem}[{{\tjucite[54, Corollary 4.11][\cor 4.11]{atiyah_introduction_1969}}}]
...
维数理论也是交换代数的重要话题, Atiyah-MacDonald \tjucite[116--126, Chapter 11][\chap 11]{atiyah_introduction_1969}一书中介绍了 Noether 局部环的维数理论, 需要模论以及完备化等技术. 限于篇幅原因, 这一节我们仅介绍域的有限生成整环的维数理论, 其与域论关系密切, 因此我们将先介绍域论中的维数理论, 即先叙述超越基与超越次数相关的内容. 环的维数可以直接利用域的超越次数来定义, 而通过对素理想的研究, 我们发现素理想是构成维数的重要组成部分, 通过考察一个素理想的高度和深度可以得出环的维数, 并且每个高度的素理想都总是存在的, 这个命题十分的强, 甚至足以证明著名的Hilbert零点定理.

% 域论的记号 代数相关代数无关 域扩张 F(S)

回顾线性空间的维数理论, 我们定义了线性空间中线性相关与线性无关的概念, 从而引出了Hamel基的概念, 并证明了Hamel基的存在性\footnotemark 及其在基数意义下的唯一性\tjucite[44--46, Section 2.1][\sect 2.1]{ciarlet_linear_2013}. 我们将推广这个工作, 定义集合论意义下的相关关系, 并将其应用在域扩张中, 从而得到域扩张的超越基与超越次数.
\footnotetext{``任意非零线性空间都有Hamel基"这个命题在\axiom{ZF}公理系统下也可以推出选择公理\axiom{AC}\tjucite{blass_existence_1984}, 从而与选择公理\axiom{AC}等价.}
...
\begin{proposition}[{{\tjucite[123, Lemma 1][\sect 3.6, \lem 1]{jacobson_basic_1989}}}]\label{prop:dependlemma}
...
\begin{theorem}[{{\tjucite[123, Theorem 3.12][\thm 3.12]{jacobson_basic_1989}}}]\label{thm:dependbase}
...
\begin{proposition}[{{\tjucite[515, Theorem 8.33][\thm 8.33]{jacobson_basic_1989}}}]\label{prop:algbdependcrit}
...
\begin{proposition}[{{\tjucite[516, Theorem 8.34][\thm 8.34]{jacobson_basic_1989}}}]\label{prop:algbdependcorr}
...
\begin{proof}
  ...
  \ref{enum:dependaxiom4} ``如果$x\depend S$但是$x\ndepend S-\{y\}$, 则$y\depend (S-\{y\})\cup\{x\}$". 设$T\coloneq S-\{y\}$, $K\coloneq F(T)$, 则$x$为$K$的超越元同时也为$K(y)$的代数元, 由\thref{prop:algbdependcrit}知$\{x, y\}$在$K$上代数相关, 因此存在非零多项式$f\in K[X, Y]$使得$f(x, y)=0$, 因为$x$为$K$的超越元, 所以$f$可以通过合并系数以及代入来诱导出多项式$\tilde{f}\in K[x](Y)$使得$\tilde{f}(y)=0$, 从而得证$y$是$K(x)=F(T\cup\{x\})$上的代数元, 即$y\depend T\cup \{x\}$.
\end{proof}
...
\begin{theorem}[{{\tjucite[516, Theorem 8.35][\thm 8.35]{jacobson_basic_1989}}}]\label{thm:tranbasis}
...
\begin{propositionnoproof}[{{\tjucite[101, Theorem 28 and 29][\thm 28 \and 29]{zariski_commutative_1958}}}]\label{prop:primeidealdim}
...
\begin{proposition}[Noether正规化引理{{\tjucite[\chap 5, \ex 16][69, Exercise 16]{atiyah_introduction_1969}}}]\label{prop:noethernormalization}
...
\begin{proposition}[{{\tjucite[91, Lemma][\chap VI, \sec 14, \lem*]{zariski_commutative_1960}}}]\label{prop:heightoneideallemma}
  设$A=\kk [x_1, \dotsc, x_n]$为$k$的有限生成整环, 超越次数为$r$, $\ideal{p}$为高度为一的素理想, 则$\codim (\ideal{p}) = \dim (A)-1$.
\end{proposition}

\begin{proof}
  这里我们同样只对$\kk$为无限域的情况来证明. 首先考虑$x_1, \dotsc, x_n$代数无关的情况, 此时$A$为$\kk$的$n$元多项式环, 从而是唯一分解整环. 而在唯一分解整环中, 利用素元和不可约元的等价性, 可以得到高度为一的素理想一定是主理想, 并且可以由一个素元生成\tjucite[174, Exercise 21.32][\ex 21.32]{altman_term_2017}, 故我们有$\ideal{p}=(f)$, $f$的次数一定为正. 不妨设$x_n$出现在了多项式$f$的形式定义之中, 则$\ideal{p}$中的每个元素都与$x_n$相关, 因此在商去$\ideal{p}$之后$x_1, \dotsc, x_{n-1}$仍在$\kk$上代数无关. 这证明了$\codim (\ideal{p})\geq n-1$, 又因为$\ideal{p}\neq (0)$故有$\codim (\ideal{p})=n-1$.

  再考虑$r<n$的情况. 利用\thref{prop:noethernormalization}我们可以找到代数无关的$y_1, \dotsc, y_r$使得$A$能由$A'=\kk [y_1, \dotsc, y_r]$整表示. 取$\ideal{p}'=\ideal{p}\cap A'$, 则$\ideal{p}'$为多项式环$A'$中的高度为一的素理想, 因此由之前证明的情况知$\codim (\ideal{p}')=r-1$, 于是$\codim (\ideal{p})=r-1$.
  % 分类讨论, 如果是多项式环的话就是唯一分解整环, 非常简单; 否则的话利用\thref{prop:noethernormalization}来处理.
\end{proof}

\begin{theorem}[{{\tjucite[193, Theorem 20][\chap 7, \sec 7, \thm 20]{zariski_commutative_1960}}}]\label{thm:ringdim}
...
\begin{proposition}[{{\tjucite[195, Corollary 3][\chap 7, \sec 7, \cor 3]{zariski_commutative_1960}}}]\label{prop:ringdimprimeidealexist}
...
\begin{theorem}[弱Hilbert零点定理{{\tjucite[82, Proposition 7.9][\prop 7.9]{atiyah_introduction_1969}}}]\label{thm:hilbert-nullstellensatz-weak}
...
% \begin{proposition}[Artin-Tate引理{{\tjucites[81, Proposition 7.8]{atiyah_introduction_1969}[Theorem 1]{artin_note_1951}}}]
...
% \begin{theorem}[弱Hilbert零点定理{{\tjucites[82, Proposition 7.9]{atiyah_introduction_1969}[Theorem 2]{artin_note_1951}}}]
%     设$\field{k}$是域, $E$为$\field{k}$的有限生成代数, 如果$E$也是域, 则$E$为$\field{k}$的有限代数域扩张. 特别地, 如果$\field{k}$为代数闭域, $A$为$\field{k}$的有限生成代数, $\ideal{m}$为$A$的极大理想, 则$A{\divslash}\ideal{m}\cong \field{k}$.
% \end{theorem}
...
\begin{theoremnoproof}[Krull主理想定理{{\tjucite[238, Theorem 29][\chap IV, \sec 14, \thm 29]{zariski_commutative_1958}}}]\label{thm:krullhauptidealsatz}
  设$(f)$为Noether整环$A$的一个非零真主理想, $\ideal{p}$为$(f)$的一个孤立伴随素理想, 则$h(\ideal{p})=1$.
\end{theoremnoproof}

在域的有限生成整环中可以得到比\thref{thm:krullhauptidealsatz}更强的结论.

\begin{propositionnoproof}[{{\tjucite[240, Theorem 30][\chap IV, \sec 14, \thm 30]{zariski_commutative_1958}}}]\label{prop:isolatedprimegenerator2height}
...
\begin{proposition}[{{\tjucite[242, Theorem 31][\chap IV, \sec 14, \thm 31]{zariski_commutative_1958}}}]\label{prop:isolatedprimeheight2generator}
...
% 在唯一分解整环中, 利用素元和不可约元的等价性, 可以得到高度为一的素理想一定是主理想, 并且可以由一个素元生成\tjucite[174, Exercise 21.32]{altman_term_2017}. 我们将要在Noether整环的情况下推广这一结论, 并应用维数理论证明\thref{prop:heightoneideallemma}的逆命题成立.

% \begin{theorem}[Krull主理想定理{{\tjucite[238, Theorem 29]{zariski_commutative_1958}}}]\label{thm:krullhauptidealsatz}
%   设$(f)$为Noether整环$A$的一个非零真主理想, $\ideal{p}$为$(f)$的一个孤立伴随素理想, 则$h(\ideal{p})=1$.
% \end{theorem}

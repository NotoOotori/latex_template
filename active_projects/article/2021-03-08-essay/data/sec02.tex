...
如果$N$是$A$-模$M$的Abel子群并且关于$A$的模运算封闭, 即对于任意$x\in N, a\in A$都有$ax\in N$, 则称$N$是$M$的\emph{子模}. $A$有自然的$A$-模结构, $\ideal{a}$为$A$的子模当且仅当为$A$的理想, 于是在这种意义下模的概念是环与理想概念地推广. 商群$M{\divslash}N$有自然的$A$-模结构, 即$a(x+N)\coloneq ax+N$, 称为$M$对$N$的\emph{商模}.
...
与有限维线性空间上的线性变换类似, 有限生成模上的自同态也可以由矩阵表示, 只不过就算给定了生成元矩阵表示也不唯一, 但是我们仍有如下命题, 是环上的Cayley-Hamilton定理的简单应用.
...
\begin{proposition}[{{\parencite[74, Proposition 6.1]{atiyah_introduction_1969}}}]\label{prop:chaincondition}
    ...
    \begin{enumerate}
        \item\label{enum:prop-chain-condition-chain} (升链条件)
        \item\label{enum:prop-chain-condition-maximal} (极大条件)
    \end{enumerate}
\end{proposition}

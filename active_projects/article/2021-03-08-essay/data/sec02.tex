...
\begin{theorem}[弱Hilbert零点定理{{\parencite[82, Proposition 7.9]{atiyah_introduction_1969}}}]
    设$\field{k}$是域, $A$为$\field{k}$的多项式函数整环, 如果$A$也是域, 则$A$为$\field{k}$的有限代数域扩张. 特别地, 如果$\field{k}$为代数闭域, $A$为$\field{k}$的多项式函数整环, $\ideal{m}$为$A$的极大理想, 则$A/\ideal{m}\cong \field{k}$.
\end{theorem}

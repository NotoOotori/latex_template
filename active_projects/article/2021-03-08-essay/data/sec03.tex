...
\begin{proposition}[{{\tjucite[2, Proposition 1.1]{hartshorne_algebraic_1977}}}]\label{prop:affinezariskitopology}
...
如果非空拓扑空间$X$不能写成两个真闭子空间的并, 则称$X$是\emph{不可约的}, 这等价于$X$中任意两个非空开集都有非空交, 也等价于$X$中每个非空开集都是稠密的\tjucite[13, Exercise 19]{atiyah_introduction_1969}. 称$X$的子集$Y$不可约当且仅当$Y$的子拓扑结构是\emph{不可约的}. 为了叙述方便, 我们不认为空集是不可约的.
...
\begin{proposition}[{{\tjucite[5, Proposition 1.5]{hartshorne_algebraic_1977}}}]\label{prop:noetherirreduciblecomponent}
...
\begin{proposition}[{{\tjucite[9, Proposition 2.1]{hartshorne_algebraic_1977}}}]
...
接下来我们将要构造$n$-射影空间中由$n$-仿射空间所构成的开覆盖, 这个构造与证明实射影空间是实微分流形中的构造\tjucite[4--5, Definition 2.4]{flaherty_riemannian_1992}完全相同. 如果$f\in S$是一个齐次线性多项式, 则称$f$的零点集为一个\emph{超平面}. 特别地, 对于$j=0, \dotsc, n$, 记$H_j$为$X_j$的零点, 并记$U_j$为开集$\PP^n-H_j$. 定义映射$\varphi_j\colon U_j\to \AF^n$, 把有齐次坐标$(x_0, \dotsc, x_n)$的点$P\in U_j$映到有仿射坐标$(x_0{\divslash}x_j, \dotsc, x_{j-1}{\divslash}x_j, x_{j+1}{\divslash}x_j, \dotsc,  x_n{\divslash}x_j)$的点$Q\in \AF^n$. 因为$x_j\neq 0$, 并且$x_i{\divslash}x_j$的取值与齐次坐标的选取无关, 因此$\varphi_j$是良定的.

\begin{proposition}[{{\tjucite[10, Proposition 2.2]{hartshorne_algebraic_1977}}}]\label{prop:projspaceopencoverhomeo}
...
\begin{proposition}[{{\tjucite[11, Corollary 2.3]{hartshorne_algebraic_1977}}}]\label{prop:projvarietyopencoverhomeo}
  如果$Y$是一个 (拟) 射影代数簇, 则$Y$可以由开集族$Y\cap U_j$开覆盖, 并且$\varphi_j$诱导了从$Y\cap U_j$到 (拟) 仿射代数簇的同胚.
\end{proposition}

\begin{proof}
  只需要对$Y$是射影代数簇的情况, 证明$Y\cap U_j$是$U_j$中的不可约闭集. 注意到$Y\cap U_j$是不可约集$Y$的开子集, 故$Y\cap U_j$中的开集恰为$Y$中包含于$U_j$的开集\tjucite[89, Lemma 16.2]{munkres_topology_2000}, 如果开集$V\subseteq Y\cap U_j$不稠密, 记$V$在$Y\cap U_j$中的闭包为$\overline{V}$, 那么$V$在$Y$中的闭包一定与$Y$的非空开集$Y\cap U_j-\overline{V}$不交\tjucite[95, Theorem 17.5]{munkres_topology_2000}, 因此与$Y$是不可约集矛盾, 故$Y\cap U_j$不可约. 再由子拓扑的定义即可得到$Y\cap U_j$是$U_j$中的闭集.
\end{proof}
...
\begin{proposition}[{{\tjucite[15, Lemma 3.1]{hartshorne_algebraic_1977}}}]\label{prop:affineregularcontinuous}
...
\begin{proposition}[{{\tjucite[15, Remark 3.1.1]{hartshorne_algebraic_1977}}}]\label{prop:projregularcontinuous}
...
将之前定义的仿射代数簇, 拟仿射代数簇, 射影代数簇, 拟射影代数簇统称为\emph{代数簇}, 我们已经定义了代数簇上的正则函数. 类似于单复变函数中非零全纯函数的零点是孤立的\tjucite[127]{ahlfors_complex_1978}, 代数簇上的函数也有很强的刚性.
...
\begin{proposition}[{{\tjucite[18, Proposition 3.3]{hartshorne_algebraic_1977}}}]\label{prop:projectiveopencoverisomorphism}
...
\begin{proposition}[{{\tjucite[20, Lemma 3.6]{hartshorne_algebraic_1977}}}]\label{prop:morphismtoaffinevariety}
...
\begin{proposition}[{{\tjucite[13, Exercise 2.13]{hartshorne_algebraic_1977}}}]\label{prop:segreembedding}
...
\begin{proposition}[{{\tjucite[22, Exercise 3.16]{hartshorne_algebraic_1977}}}]\label{prop:projectiveproduct}

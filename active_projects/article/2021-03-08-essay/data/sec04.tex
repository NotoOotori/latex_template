...
\begin{theorem}{{\parencite[19, Proposition 3.5]{hartshorne_algebraic_1977}}}\label{thm:categoryisoaffinevariety}
    ...
    \begin{equation*}
        \begin{tikzcd}[sep=small]
            ...
            % \arrow[r, mapsto, from=U, to=V, inner sep=small]
        \end{tikzcd}
    \end{equation*}
...
\begin{proof}
    ...
    反过来, 如果给定了$\kk$-代数同态$h\colon A(Y)\to \OO (X)$. 将$Y$嵌入到某个$\AF^n$中, 假设$Y$为$\AF^n$的不可约闭子集, 于是$A(Y)=\kk [X_1, \dotsc, X_n]{\divslash} I(Y)$. 记$X_j\in \OO (Y)\cong A(Y)$为单项式$X_j\in A$所诱导的$Y$上的正规函数. 考虑$\xi_j=h(X_j)\in \OO (X)$, 是$X$上的正规函数, 于是我们可以定义$\psi\colon X\to \AF^n$将$P\in X$映到$(\xi_1 (P), \dotsc, \xi_n (P))\in \AF^n$.
    ...
    最后证明范畴等价. 我们已经证明了这个函子是fully faithful的, 又因为每个$k$的多项式函数整环都可以写成某个$k$的代数簇的仿射坐标环, 因此这个函子也是essentially surjective的, 于是由\thref{thm:categoryequivalence}知这个函子给出了范畴之间的等价.
\end{proof}

运用这个范畴等价, 我们可以得到更加深刻的结果.

...
\begin{theorem}[{{\parencite[11--12, Exercise 2.6; 18, Theorem 3.4]{hartshorne_algebraic_1977}}}]\label{thm:projectiverings}
    ...
    \begin{enumerate}
        \item $\dim (S(Y))=\dim (Y)+1$;
        \item $\OO (Y)\cong\kk$;
        \item 对于每个点$P\in Y$, 令$\ideal{m}_P\subseteq S(Y)$为在$P$点取值为零的齐次坐标函数所生成的极大理想, 则$\OO_P\cong S(Y)_{(\ideal{m}_P)}$;
        \item $K(Y)\cong S(Y)_{(0)}$.
    \end{enumerate}
\end{theorem}

% 我们将要证明某个范畴等价, 为此我们需要正规函数的刻画这样一个引理

...
\begin{document}
    ...
    \begin{problem}[秦九韶算法]
        实现秦九韶算法, 
        并在阶数$n$变化的情况下与直接计算一元多项式比较浮点运
        算量和计算时间.
    \end{problem}

    \begin{solution}
        秦九韶算法为迭代求解多项式在某一点值的算法, 实现代码如下.
        \lstinputlisting[
            title=秦九韶算法,
            style=MATLAB-editor,
        ...
        假设多项式次数为$n$, 那么用秦九韶算法计算多项式的值
        需要作$n$次乘法和$n$次加法, 而用多项式定义式计算需要作
        $n$次乘法$n-1$次乘方和$n$次加法, 秦九韶算法在运算量上有优越性.

        我们令~P=linspace(1, 2, 1000001)~为1000000次多项式, 用MATLAB内置的
        运行与计时计算$P$在点0.9的值并测速. 最后发现我们编写的秦九韶算法程序
        花费了0.069秒, MATLAB内置的~polyval~函数花费了0.013秒, 而直接计算
        花费了0.115秒.
    \end{solution}

    \begin{problem}[牛顿插值]
        结合符号运算, 实现牛顿插值法, 给出牛顿插值多项式
        (而非插值多项式在某点处的值).
    \end{problem}

    \begin{solution}
        我们采用待定系数法求出牛顿多项式的系数, 代码如下.
        \lstinputlisting[
            title=牛顿插值,
            style=MATLAB-editor,
            basicstyle=\mlttfamily\scriptsize

...
\begin{document}
    ...
    \section{Newton-Cotes公式求积分}

    MATLAB代码如下:

    \lstinputlisting[
        caption=newton\_cotes.m,
        style=MATLAB-editor,
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{newton_cotes.png}
        \caption{Newton-Cotes公式积分余项}
    \end{figure}

    由图像可见积分余项$R_{n}$并非收敛. 这是因为当$n\geq 8$的时候,
    Cotes系数有正有负, 此时求积公式的稳定性得不到保证, 收敛性一般不成立.

    \section{复合公式和Gauss公式求积分}
    ...
    \lstinputlisting[
        label=CompositeSimpson,
        caption=composite\_simpson.m,
        style=MATLAB-editor,
        basicstyle=\mlttfamily\scriptsize
    ]{composite_simpson.m}

    其中Gauss-Legendre公式代码如下:

    \lstinputlisting[
        caption=gauss\_legendre.m,
        style=MATLAB-editor,
        basicstyle=\mlttfamily\scriptsize
    ]{gauss_legendre.m}

    发现复合Simpson求积公式和Gauss-Legendre公式均在$n=7$的时候误差便小于$10^{-8}$,
    可见它们的收敛速度均很快.
    ...
    \lstinputlisting[
        caption=composite\_trapezoid.m,
        style=MATLAB-editor,
        basicstyle=\mlttfamily\scriptsize
    ]{composite_trapezoid.m}

    其中复合Simpson求积公式代码见代码\ref{CompositeSimpson}.
    ...
    \begin{lstlisting}[
        ...
        n1 = [1873 365 466 548 620]
        n2 = [466 19 8 9 12]
    \end{lstlisting}

    可见可导并且导数变化量不大的函数积分收敛的比较快.
    ...
    两点公式得到的导数的平均值为\matlabinline{1.9489},
    三点公式得到的导数的平均值为\matlabinline{1.9369}.
